...
The amount you put in is capped at your **available** balance on Hyperliquid (free margin, or free USDC in a unified account): you cannot type more than you have, and the 25% / 50% / 75% / Max shortcuts are shares of that available balance. Under the slider, a card shows the **minimum capital for this copy** in large type, next to what you have available. It turns amber when your amount is below it: below the minimum only the trader's larger trades get copied (see [Exchange minimum and skipped positions](#exchange-minimum-and-skipped-positions)); at or above it, all of their current trades are copied. The minimum is computed from the trader's smallest open position relative to their equity, so it changes from trader to trader.
