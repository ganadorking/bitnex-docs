...
4. **The engine copies 24/7** — it mirrors the trader's **new** position changes (opens, increases, reductions, closes **and their TP/SL**), proportional to your allocation. Every copied order and position carries the word "Copy" at the end of its row in your tables (positions, open orders, fills and order history); on a position it links to the trader you are copying.
...
The amount you put in is capped at your **available** balance on Hyperliquid (free margin, or free USDC in a unified account): the slider runs from 1% to 100% of that balance (with no balance it stays at 0), and you can type an exact amount in the box next to it, never more than you have. Under the slider, one line shows the **minimum required** for this copy and your available balance. The minimum turns amber when your amount is below it: below the minimum only the trader's larger trades get copied (see [Exchange minimum and skipped positions](#exchange-minimum-and-skipped-positions)); at or above it, all of their current trades are copied. It is computed from the trader's smallest open position relative to their equity, so it changes from trader to trader. Copying several traders at once (MultiCopy) is not part of this popup; it will get its own configuration.
...
The tab opens with a status line (copy engine running and checking every ~5 s, or not reporting) and a reminder of the one rule that surprises most people: only **new** trades after you start are copied, never the trader's existing portfolio. Each copy then states in one sentence what the engine is doing with it right now (waiting for the trader's next trade, copying with N positions open, or paused), and shows the trader's name (or address), when you started copying, and four numbers: your allocation, the copy's PnL, copied volume and copied orders. Your settings (max leverage, max worse entry, protection) sit in one line below, and the positions this copy opened appear as chips with their side. Once the engine has copied at least one order for that trader, the same line shows **how far your fills landed from the trader's price**, in basis points (a positive number means you entered worse) and how many fills that average covers. It is telemetry, not a limit: the "max worse entry" setting is what actually stops a copy from filling too far away. Each copy also remembers what the trader looked like when you started (equity, 30-day ROI, volume and drawdown). If they change regime, an amber line says so: equity down by more than half, 30-day drawdown up by ten points or more, or 30-day volume down to less than a fifth. Nothing is done automatically; it is your call to pause, adjust or stop. The **Pause** and **Stop** buttons are labelled; both ask what to do with open positions before acting.
